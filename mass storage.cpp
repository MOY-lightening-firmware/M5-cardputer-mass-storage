...
// ─── Global Nesne Tanımları ───────────────────────────────────────────────────
USBMSC   MSC;
SPIClass sdSPI(HSPI);

// ─── Global Durum Değişkeni Tanımları ────────────────────────────────────────
...
bool sdRawInit() {
    sdSPI.beginTransaction(
        SPISettings(400000, MSBFIRST, SPI_MODE0));
    sdDeselect();
...
bool sdReadSectors(uint8_t* buf, uint32_t lba, uint32_t count) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        sdTransfer(0x40 | 17);
        sdTransfer((addr >> 24) & 0xFF);
        sdTransfer((addr >> 16) & 0xFF);
        sdTransfer((addr >>  8) & 0xFF);
        sdTransfer((addr      ) & 0xFF);
        sdTransfer(0xFF);
        ...
        for (int b = 0; b < SECTOR_SIZE; b++)
            buf[i * SECTOR_SIZE + b] = sdTransfer(0xFF);
...
bool sdWriteSectors(const uint8_t* buf, uint32_t lba, uint32_t count) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        for (int b = 0; b < SECTOR_SIZE; b++)
            sdTransfer(buf[i * SECTOR_SIZE + b]);
...
int32_t onRead(uint32_t lba, uint32_t offset,
    ...
    if (count == 0) return -1;
    return sdReadSectors((uint8_t*)buffer, lba, count)
        ? (int32_t)bufsize : -1;
}
...
void drawSDCardShape(int x, int y, int w, int h, uint16_t col) {
    ...
    for (int i = 0; i < 6; i++) {
        M5Cardputer.Display.fillRect(
            pinStart + i * pinGap, pinY, pinW, 10, C_BLACK);
    }
}
...
void animDataFlow() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        uint16_t c = M5Cardputer.Display.color565(
            0, br / 5, br / 2);
